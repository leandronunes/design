...
module ActsAsDesignBlock
  ...
  module ClassMethods
    ...
    def acts_as_design_block(config = {})
      require 'design/editor/block_helper'
      include ActsAsDesignBlock::InstanceMethods
      include Design
      helper Design::BlockHelper
      helper Design::BlockEditHelper 
  ...
  module InstanceMethods
    ...
    def design_load_block
      @design_block = design_content.blocks.find(params[:block_id])
      raise "block cannot be loaded to design %s with parameter %s" % [design_content.inspect, params[:block_id]] if @design_block.nil?
    end

    def design_editor_destroy_block
      block_id = params[:block_id]
      raise "This block cannot be removed" unless design_content.blocks.find(block_id).destroy
      render :nothing => true
    end
