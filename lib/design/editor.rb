...
#    end
    ...
    def design_editor_add_block
      ...
      #
      # design_editor_render_local_rjs
    ...
    # FIXME: move actions in edit_template from anhetegua to here

    private 

    def design_editor_render_local(extension = 'rhtml', layout = true)
      ...
      if File.exists?(view_path)
        ...
        render :file => File.join(File.dirname(__FILE__), '..', '..', 'views', 'design_editor', params[:action] + '.' + extension), :layout => layout
      end
    end

    def design_editor_render_local_rjs
      design_editor_render_local('rjs', false)
    end
    ...
    def design_editor_set_icon_theme
      if request.post? && design_editor_exists_icon_theme?(params[:icon_theme])
